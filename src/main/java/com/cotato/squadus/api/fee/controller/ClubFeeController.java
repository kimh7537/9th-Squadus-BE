...
public class ClubFeeController {
	...
	public ResponseEntity<ClubFeeSummaryResponseList> findAllClubFeesSummary(
		@AuthenticationPrincipal CustomOAuth2Member customOAuth2Member, @PathVariable("clubId") Long clubId) {
		ClubFeeSummaryResponseList clubFeeSummaryResponseList = clubFeeService.findAllClubFeeTypesSummary(
			customOAuth2Member, clubId);
		return ResponseEntity.ok(clubFeeSummaryResponseList);
	}

	@PostMapping("")
	@Operation(summary = "동아리 회비 등록", description = "동아리 회비를 등록합니다.")
	public ResponseEntity<ClubFeeCreateResponse> createClubFee(
		@AuthenticationPrincipal CustomOAuth2Member customOAuth2Member, @PathVariable("clubId") Long clubId,
		ClubFeeCreateRequest clubFeeCreateRequest) {
		ClubFeeCreateResponse createdClubFee = clubFeeService.createFee(customOAuth2Member, clubId,
	...
	public ResponseEntity<ClubFeeUsageResponseList> findAllClubFeeUsage(
		@AuthenticationPrincipal CustomOAuth2Member customOAuth2Member, @PathVariable("clubId") Long clubId) {
		ClubFeeUsageResponseList clubFeeUsageResponseList = clubFeeService.findAllClubFeeUsage(customOAuth2Member,
			clubId);
	...
	public ResponseEntity<ClubFeeUsageResponseList> findAllClubFeeUsageByFeeTypeId(
		@AuthenticationPrincipal CustomOAuth2Member customOAuth2Member, @PathVariable("clubId") Long clubId,
		@PathVariable("feeTypeId") Long feeTypeId) {
		ClubFeeUsageResponseList clubFeeUsageResponseList = clubFeeService.findAllClubFeeUsageByFeeTypeId(
			customOAuth2Member, clubId, feeTypeId);
		return ResponseEntity.ok(clubFeeUsageResponseList);
	}

	@PostMapping("/{feeTypeId}/usage")
	@Operation(summary = "동아리 회비 사용", description = "회비의 id를 지정하여 동아리 회비를 사용합니다.")
	public ResponseEntity<ClubFeeUsageCreateResponse> createClubFeeUsage(
		@AuthenticationPrincipal CustomOAuth2Member customOAuth2Member, @PathVariable("clubId") Long clubId,
		@PathVariable("feeTypeId") Long feeTypeId, ClubFeeUsageRequest clubFeeUsageRequest) {
		ClubFeeUsageCreateResponse clubFeeUsage = clubFeeService.createClubFeeUsage(customOAuth2Member, clubId,
			feeTypeId, clubFeeUsageRequest);
	...
	public ResponseEntity<ClubFeePaymentInfoResponseList> findClubFeePaymentInfo(
		...
		@PathVariable("feeTypeId") Long feeTypeId) {
		ClubFeePaymentInfoResponseList clubFeePaymentInfo = clubFeeService.findClubFeePaymentInfo(customOAuth2Member,
			clubId, feeTypeId);
	...
	public ResponseEntity<ClubFeePaymentUpdateResponse> updateClubFeePaymentInfo(
		...
		ClubFeePaymentUpdateResponse clubFeePaymentUpdateResponse = clubFeeService.updateClubFeePaymentInfo(
			customOAuth2Member, clubId, feeTypeId, clubFeePaymentUpdateRequest);
