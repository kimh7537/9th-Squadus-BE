...
public class RecruitingPostController {
	...
	public ResponseEntity<Page<RecruitingPostResponse>> findAllRecruitingPosts(
		@AuthenticationPrincipal CustomOAuth2Member customOAuth2Member, @PageableDefault(size = 10) Pageable pageable) {
		Page<RecruitingPostResponse> allRecruitingPosts = recruitingPostService.findAllRecruitingPosts(
			customOAuth2Member, pageable);
	...
	public ResponseEntity<RecruitingPostInfoResponse> findRecruitingPostByPostId(
		@AuthenticationPrincipal CustomOAuth2Member customOAuth2Member, @PathVariable Long recruitingPostId) {
		RecruitingPostInfoResponse recruitingPostByPostId = recruitingPostService.findRecruitingPostByPostId(
			customOAuth2Member, recruitingPostId);
		return ResponseEntity.ok(recruitingPostByPostId);
	}

	@PostMapping
	@Operation(summary = "동아리 홍보글 생성", description = "동아리 홍보글을 생성합니다")
	public ResponseEntity<RecruitingPostCreateResponse> createRecruitingPost(
		...
		RecruitingPostCreateResponse recruitingPost = recruitingPostService.createRecruitingPost(customOAuth2Member,
			recruitingPostCreateRequest);
