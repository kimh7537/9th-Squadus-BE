...
import com.cotato.squadus.api.match.dto.matchPost.request.MatchRequestRequest;
...
public class MatchService {
	...
	public MatchCreateResponse createMatch(MatchCreateRequest matchCreateRequest) {
		...
		// 해당 club의 임원인지 확인
		if (!clubAdminMemberRepository.findActiveAdminByClubIdAndClubMemberId(homeClub.getClubId(),
			matchCreateRequest.getClubMemberId()).isPresent()) {
	...
	public List<MatchCreateResponse> findAllMatches() {
		...
		return matchPostRepository.findAll().stream()
			.filter(matchPost -> {
				LocalDateTime matchDateTime = LocalDateTime.of(matchPost.getMatchStartDate(),
	...
	public MatchRequestResponse sendMatchRequest(MatchRequestRequest matchRequestRequest) {
		...
		// 해당 club의 임원인지 확인
		if (!clubAdminMemberRepository.findActiveAdminByClubIdAndClubMemberId(clubMember.getClub().getClubId(),
			clubMember.getClubMemberIdx()).isPresent()) {
			throw new AppException(ErrorCode.CLUB_ACCESS_DENIED);
		}
	...
	public MatchCreateResponse updateMatchPost(Long matchPostId, MatchCreateRequest matchCreateRequest) {
		...
		matchPost.update(
			...
			new MatchPlace(matchCreateRequest.getMatchPlace().getCity(),
				matchCreateRequest.getMatchPlace().getDistrict()),
	...
	public MatchCreateResponse deleteMatchPost(Long matchIdx, Long clubMemberId) {
		...
		boolean isAdmin = clubAdminMemberRepository.findActiveAdminByClubIdAndClubMemberId(homeClub.getClubId(),
				clubMemberId)
			.isPresent();
