...
public class ClubScheduleService {
	...
	public List<ClubScheduleResponse> findSchedulesByYearMonth(Long clubId, int year, int month) {
		...
		List<ClubSchedule> schedules = clubScheduleRepository.findByClubClubIdAndDateBetween(clubId, startDate,
			endDate);
		return schedules.stream()
			.map(ClubScheduleResponse::from)
			.collect(Collectors.toList());
	...
	public ClubScheduleResponse createSchedule(Long clubId, ClubScheduleRequest scheduleRequest) {
		...
		ClubAdminMember adminMember = clubAdminMemberRepository.findByClubMemberIdxAndAdminStatus(
				scheduleRequest.getAuthorId(), AdminStatus.CURRENT)
			.orElseThrow(
				() -> new EntityNotFoundException("Admin member not found with id: " + scheduleRequest.getAuthorId()));
	...
	public void deleteSchedule(Long clubId, Long scheduleId, Long adminId) {
		...
		ClubAdminMember adminMember = clubAdminMemberRepository.findByClubMemberIdxAndAdminStatus(adminId,
				AdminStatus.CURRENT)
			.orElseThrow(
				() -> new EntityNotFoundException("Admin member not found or not CURRENT with id: " + adminId));
	...
	public ClubScheduleResponse updateSchedule(Long clubId, Long scheduleId, ClubScheduleRequest scheduleRequest) {
		...
		ClubAdminMember adminMember = clubAdminMemberRepository.findByClubMemberIdxAndAdminStatus(
			...
			.orElseThrow(() -> new EntityNotFoundException(
				"Admin member not found or not CURRENT with id: " + scheduleRequest.getAuthorId()));
