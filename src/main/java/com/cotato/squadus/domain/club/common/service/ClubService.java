...
public class ClubService {
	...
	@Transactional
	public ClubApplyResponse joinClub(CustomOAuth2Member customOAuth2Member, Long clubId,
		ClubApplyRequest clubApplyRequest) {
	...
	@Transactional
	public ClubUpdateResponse updateClub(CustomOAuth2Member customOAuth2Member, Long clubId,
		ClubUpdateRequest clubUpdateRequest, MultipartFile logoImage) {
		...
		if (clubIds.size() < 5) {
			return clubIds.stream()
				.map(clubId -> clubRepository.findById(Long.parseLong(clubId))
					.map(club -> new ClubRankResponse(club.getClubName(), club.getLogo(), club.getMatchScore(),
						club.getClubRank(), 0))
					.orElseThrow(() -> new EntityNotFoundException("동아리를 찾을 수 없습니다.")))
	...
	//특정 동아리에서 matchScore가 업데이트 될 때, tier도 변경
	public List<ClubRankResponse> updateTier(SportsCategory category) {
		String key = "ranking:" + category.name();
		Set<String> clubIds = redisTemplate.opsForZSet().reverseRange(key, 0, -1); // 높은 점수 순으로 가져옴
