package com.cotato.squadus.domain;
...
public class S3ImageService {
    ...
    private String uploadImage(MultipartFile image) {
        ...
        log.info("Uploading image2 " + image.getOriginalFilename());
        try {
    ...
    private void validateImageFileExtention(String filename) {
        ...
        log.info("Validating image file " + filename);
    }
    ...
    private String uploadImageToS3(MultipartFile image) throws IOException {
        ...
        log.info("hello image " + image.getOriginalFilename());

        try{
            PutObjectRequest putObjectRequest =
                    new PutObjectRequest(bucketName, s3FileName, byteArrayInputStream, metadata)
                            .withCannedAcl(CannedAccessControlList.PublicRead);
            log.info("first image " + image.getOriginalFilename());
            amazonS3.putObject(putObjectRequest); // put image to S3
            log.info("second image " + image.getOriginalFilename());
        ...
        log.info("upload image " + originalFilename + " successfully uploaded");
